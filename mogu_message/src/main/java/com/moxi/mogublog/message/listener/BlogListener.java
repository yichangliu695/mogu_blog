...
import com.moxi.mogublog.message.global.RedisConf;
import com.moxi.mogublog.message.global.SysConf;
        ...
        if (map != null) {
            ...
            try {
                switch (comment) {
                    ...
                    case SysConf.DELETE: {
                        log.info("mogu-message处理删除博客: uid:" + uid);
                        updateSearch(map);
                        if (ESearchModel.ES.equals(searchModel)) {
                            // 增加ES索引
                    ...
                    break;
                    default: {
                        log.info("mogu-message处理博客兜底方法");
                    }
                }
            } catch (Exception e) {
                e.printStackTrace();
                log.error("【mogu-message】出现异常，请查看mogu-search是否启动！searchModel: " + searchModel);
            }
        }
    }
